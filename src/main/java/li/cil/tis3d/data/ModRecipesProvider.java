...
public final class ModRecipesProvider extends RecipeProvider {
    ...
    protected void buildRecipes(final RecipeOutput consumer) {
        ...
        ShapelessRecipeBuilder
            ...
            .save(consumer, API.resource(Items.QUEUE_MODULE.getId().getPath() + "/from_stack"));
        ShapelessRecipeBuilder
            ...
            .save(consumer, API.resource(Items.STACK_MODULE.getId().getPath() + "/from_queue"));
    }

    private static ShapedRecipeBuilder module(final DeferredHolder<Item, ? extends Item> module, final int count, final Item item, final Criterion<InventoryChangeTrigger.TriggerInstance> unlockedBy) {
        return module(module, count, unlockedBy)
            .define('S', item);
    }

    private static ShapedRecipeBuilder module(final DeferredHolder<Item, ? extends Item> module, final int count, final TagKey<Item> tag, final Criterion<InventoryChangeTrigger.TriggerInstance> unlockedBy) {
        return module(module, count, unlockedBy)
            .define('S', tag);
    }

    private static ShapedRecipeBuilder module(final DeferredHolder<Item, ? extends Item> module, final int count, final Criterion<InventoryChangeTrigger.TriggerInstance> unlockedBy) {
